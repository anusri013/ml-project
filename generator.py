...
class ResidualBlock(nn.Module):
    def __init__(self, channels: int):
        super().__init__()
        self.block = nn.Sequential(
            ConvBlock(channels, channels, kernel_size=3, padding=1),  # Same convolution - Reduced channels
            ConvBlock(channels, channels, use_act=False, kernel_size=3, padding=1)
            # Same convolution - Increased channels
        )
...
class Generator(nn.Module):
    def __init__(self, img_channels: int = 3, n_features: int = 64, n_residuals: int = 9):
        super().__init__()
        self.initial = nn.Sequential(
            nn.Conv2d(img_channels, n_features, kernel_size=7, padding=3, padding_mode='reflect'),
        ...
        self.upsample_blocks = nn.ModuleList([
            ConvBlock(n_features * 4, n_features * 2, down_sample=False, kernel_size=3, stride=2, padding=1,
                      output_padding=1),
            ConvBlock(n_features * 2, n_features * 1, down_sample=False, kernel_size=3, stride=2, padding=1,
                      output_padding=1),
        ])
...
if __name__ == '__main__':
    import torch
    # This is a test to check the dimensions
